function data = read_lv1_v3(infile)
...
%%%%%%%%%%%%%%%% open file
    ...
    if ~isempty(strfind(infile, 'lv0')) || ~isempty(strfind(infile, 'LV0'))
        disp('This is reader for lv1-file, change your file type')
        return
    end
    ...
    data.starttime = int32(fread(fid,1,'uint')); % time of first sample
    data.endtime = int32(fread(fid,1,'uint')); % time of last sample
    data.progno = int32(fread(fid,1,'int')); % program number, as definded in the host-pc software
    data.modelno = int32(fread(fid,1,'int')); % =0 singel pol., = 1 dual pol, 2 = dual pol LDR config
    ...
    data.SupPoweLev = fread(fid,1,'char*1'); % flag indicating, if power levelling has been used (0=yes, 1=no)
    data.SpkFilEna = fread(fid,1,'char*1'); % flag indicating, if spike/plankton filter has been used (0=yes, 1=no)
    data.PhaseCorr = fread(fid,1,'char*1');% flag indicating, if phase correction (dual. pol. radars) has been used (0=yes, 1=no)
    data.RelPowCorr = fread(fid,1,'char*1'); % flag indicating, if relative power correction (dual. pol. radars) has been used (0=yes, 1=no)
    data.FFTWindow = fread(fid,1,'char*1'); % FFT window in use: 0 = SQUARE, 1 = PARZEN, 2 = BLACKMAN, 3 = WELCH, 4 = SLEPIAN2, 5 = SLEPIAN3
    data.FFTInputRng = int32(fread(fid,1,'int')); % ADC input voltage range (+/-) [mV]
